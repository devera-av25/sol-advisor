...
installs the eight native custom-agent profiles, and validates exact model/reasoning
...
git -C "$repo_root" rev-parse --is-inside-work-tree >/dev/null 2>&1 || fail "this script must run from a Git checkout: $repo_root"
...
for required in \
  ...
  "$plugin_root/agents/sol-advisor-terra-high-consultant.toml" \
  "$plugin_root/agents/sol-advisor-sol-low-consultant.toml" \
  "$plugin_root/agents/sol-advisor-sol-medium-consultant.toml" \
...
if marketplace_output=$(codex plugin marketplace add "$repo_root" 2>&1); then
  ...
  printf '%s\n' "WARNING: marketplace add did not succeed; checking existing registration." >&2
...
if plugin_add_output=$(codex plugin add sol-advisor@sol-advisor 2>&1); then
  ...
  printf '%s\n' "WARNING: plugin add did not succeed; checking existing installation." >&2
  [ -z "$plugin_add_output" ] || printf '%s\n' "$plugin_add_output" >&2
fi
...
[ "$match_count" -eq 1 ] || fail "expected exactly one installed sol-advisor@sol-advisor entry, found $match_count."
...
for relative in \
  ...
  agents/sol-advisor-terra-high-consultant.toml \
  agents/sol-advisor-sol-low-consultant.toml \
  agents/sol-advisor-sol-medium-consultant.toml \
...
do
  ...
  cmp -s "$plugin_root/$relative" "$plugin_dir/$relative" || fail "installed plugin does not match this checkout at $relative."
done
...
note "Installing eight native custom-agent profiles"
...
terra_high=$agents_dir/sol-advisor-terra-high-consultant.toml
sol_low=$agents_dir/sol-advisor-sol-low-consultant.toml
sol_medium=$agents_dir/sol-advisor-sol-medium-consultant.toml
...
check_line "$terra_high" 'name = "sol_advisor_terra_high_consultant"'
check_line "$terra_high" 'model = "gpt-5.6-terra"'
check_line "$terra_high" 'model_reasoning_effort = "high"'
check_line "$sol_low" 'name = "sol_advisor_sol_low_consultant"'
check_line "$sol_low" 'model = "gpt-5.6-sol"'
check_line "$sol_low" 'model_reasoning_effort = "low"'
check_line "$sol_medium" 'name = "sol_advisor_sol_medium_consultant"'
check_line "$sol_medium" 'model = "gpt-5.6-sol"'
check_line "$sol_medium" 'model_reasoning_effort = "medium"'
...
printf '%s\n' '  Luna implementer low       -> gpt-5.6-luna / low'
printf '%s\n' '  Luna implementer medium    -> gpt-5.6-luna / medium'
printf '%s\n' '  Terra implementer          -> gpt-5.6-terra / medium'
printf '%s\n' '  Terra consultant           -> gpt-5.6-terra / high'
printf '%s\n' '  Sol consultant low         -> gpt-5.6-sol / low'
printf '%s\n' '  Sol consultant medium      -> gpt-5.6-sol / medium'
printf '%s\n' '  Sol implementer            -> gpt-5.6-sol / high'
printf '%s\n' '  Sol final reviewer         -> gpt-5.6-sol / high / requested read-only'
printf '\n%s\n' 'NEXT STEP: fully quit/restart Codex Desktop, then start a NEW task with GPT-5.6 Terra / Medium.'
printf '%s\n' 'Do not test newly installed roles in the bootstrap task; discovery happens in a fresh task.'
